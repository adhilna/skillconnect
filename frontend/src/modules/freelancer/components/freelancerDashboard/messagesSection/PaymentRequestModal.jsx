...
const PaymentRequestModal = ({
  ...
  const handleClose = () => {
    if (isLoading) return;
    setFormData({ amount: '', description: '', paymentMethod: '' });
    setErrors({});
    setSubmitError('');
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
      <div className="bg-black/40 backdrop-blur-xl border border-white/20 rounded-3xl p-4 sm:p-6 w-full max-w-md mx-auto max-h-[90vh] overflow-y-auto relative" style={{ zIndex: 10000 }}>
        {/* Header */}
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <h3 className="text-lg sm:text-xl font-bold text-white">Request Payment</h3>
          <button onClick={handleClose} disabled={isLoading} className="text-white/60 hover:text-white transition-colors p-1 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div className="space-y-3 sm:space-y-4">
          {/* Amount */}
          <div>
            <label className="block text-white/80 text-xs sm:text-sm font-medium mb-1 sm:mb-2">Amount *</label>
            <div className="relative">
              <input
                ...
                disabled={isLoading}
                className={`w-full bg-white/5 border rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base text-white placeholder-white/50 focus:outline-none transition-colors pr-8 sm:pr-10 ${errors.amount ? 'border-red-500 focus:border-red-400' : 'border-white/10 focus:border-purple-500'}`}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={500}
              className={`w-full bg-white/5 border rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base text-white placeholder-white/50 focus:outline-none transition-colors resize-none ${errors.description ? 'border-red-500 focus:border-red-400' : 'border-white/10 focus:border-purple-500'}`}
          ...
          {/* Payment Method */}
          <div>
            <label className="block text-white/80 text-xs sm:text-sm font-medium mb-1 sm:mb-2">Preferred Payment Method *</label>
            <select
              value={formData.paymentMethod}
              onChange={(e) => handleInputChange('paymentMethod', e.target.value)}
              disabled={isLoading}
              className={`w-full bg-white/5 border rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base text-white focus:outline-none transition-colors ${errors.paymentMethod ? 'border-red-500 focus:border-red-400' : 'border-white/10 focus:border-purple-500'}`}
            >
          ...
          <div className="flex space-x-2 sm:space-x-3 pt-2 sm:pt-4">
            ...
            <button
              ...
              disabled={isLoading || !isFormValid || !!submitError} // ✅ disable if submitError exists
